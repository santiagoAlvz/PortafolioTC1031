...
string Registro::dateToString(){
	...
	char res[20];
	strftime(res,20,"%b %e %T",&ts);
	return res;
}

string Registro::toStringSingleWhitespaces(){
	//Convierte todo el registro en una cadena, con espaciado simple
	struct tm ts = *localtime(&this->fecha);
	char res[20];
	strftime(res,20,"%b %d %T",&ts);
	return string(res)+' '+this->ip+' '+this->razon;
}
