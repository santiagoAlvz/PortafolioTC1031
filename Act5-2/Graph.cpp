...
Graph::Graph(std::string fileName){ //Lee el archivo para encontrar los valores de las IP. Complejidad O(n^2)
  this->tabla = HashTable<int, IPGrado> (13381);
  ...
  this->sortDirections(direcciones,1,this->numNodes);
...
void Graph::sortDirections(std::vector<IP> &direcciones,int l, int r){ //Complejidad O(n^2)
  //Algoritmo de Ordenamiento QuickSort
	
	int pi;
	if(l < r){
		//Divide el arreglo y hace una llamada recursiva
		pi = particion(direcciones,l, r);
		sortDirections(direcciones,l,pi-1);
		sortDirections(direcciones,pi+1,r);
	}
}

int Graph::particion(std::vector<IP> &direcciones, int l, int r){ //Complejidad O(n)
...
void Graph::createTable(){ //Complejidad O(n^2)
  int hijos, padres;
  IP direccion;
  int max = 0;
...
void Graph::getIPSummary(std::string direccion){ //Complejidad O(n)
  IP miIp(direccion);
  ...
  vector<IP> descendientes;

  for(int i = 0; i < adjList[index].size(); i++){
    descendientes.push_back(adjList[index][i]);
  }

  this->sortDirections(descendientes,1,descendientes.size()-1);

  for(int i = 0; i < descendientes.size(); i++){
    cout<<"Sucesor "<<i+1<<": "<<descendientes[i]<<endl;
  }
